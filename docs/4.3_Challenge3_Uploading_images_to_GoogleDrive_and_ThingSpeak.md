...
3. **In the Arduino code (`.ino` file):** Locate the `writeApiKey` constant and replace the placeholder with your ThingSpeak channel's Write API Key.

<img src = "./images/your_thingspeak_writeApiKey.png">

After uploading the final code to your ESP32-S3, trigger an image capture. The new image will appear in your ThingSpeak channel visualization.
...
Before integrating the image upload functionality into the main application, let's examine the project structure of `10_Images_Upload_GoogleDrive_ThingSpeak.ino`. We can see that its functions are split into separate files, as summarized in the following table.

| Filename                           | Features                                                |
| ---------------------------------- | ------------------------------------------------------- |
| google_drive.cpp, google_drive.h   | Image upload to Google Drive                            |
| thingspeak.cpp, thingspeak.h       | Upload URL to ThingSpeak channel                        |
| sd_read_write.cpp, sd_read_write.h | APIs for SD Card access                                 |
| camera_api.cpp, camera_api.h       | APIs for the camera sensor                              |
| app_httpd.cpp, app_httpd.h         | API for the camera web server (HTTP)                    |

Splitting the program into separate files with descriptive filenames ensures that we can test each major component separately, leading to a smoother development experience and a more robust final project.

We will follow this same pattern by creating three more modules:

1. `water_pump_control.cpp` and `water_pump_control.h` 

2. `sensor.cpp` and `sensor.h`

3. `led_blinky.cpp` and `led_blinky.h`

This modular approach will help keep the main project file clean and focused on the primary application logic.

**Procedures:**

1. Save `10_Images_Upload_GoogleDrive_ThingSpeak.ino`to a new project e.g. `11_Images_Upload_WaterPump_Control.ino`

   <img src = "./images/Save_project10_to_project11.png">

2. From the newly created Arduino project, click on the extension button **[...]** at the top right corner, click **New Tab**.

   <img src = "./images/add_new_files_from_new_tab.png">
   
3. Name the file as `water_pump_control.cpp`, click **OK**. Repeat this step to create a new file as `water_pump_control.h`
   
   <img src = "./images/create_water_pump_control.png">
   
4. [TODO: finish the integration task]
