...
Splitting the program into separate files with easy-to-understand filenames ensures that we can test each major component separately, leading to a smoother development experience and a more robust final project.

We will follow this same pattern by creating three more modules:

1. `control_water_pump.cpp` and `control_water_pump.h` 
