...
In this challenge, we will integrate a camera into our main application to capture and upload images to the cloud. To make this complex task more manageable and easier to debug, we will approach it in three parts.
...
Proceed to Part 2 if there is no error message.

---

### Part 2: Preparing Google Apps Script and MATLAB Script

**The source code for this part is available in the `Code/10_Images_Upload_GoogleDrive_ThingSpeak/` folder. This code serves as a standalone example for testing the image upload functionality before integrating it into the main application.**
...
To display the image in ThingSpeak, you first need to add a new channel to store the URL returned from Google Drive. For this example, we will use **Field 2** and name it **ESP32_Image**. Make a note of this name, as you will need it for the MATLAB script.
...
url = string(data.ESP32_Image); % ESP32_Image must match the field name you defined in your channel
img = imread(url);
imshow(img);
title('Latest Uploaded Image');
```

**In the MATLAB script above:** Replace `YOUR_CHANNEL_ID` and `YOUR_READ_API_KEY` with your actual ThingSpeak channel ID and Read API Key. Also, ensure the `ESP32_Image` string in `string(data.ESP32_Image)` matches the name you gave to Field 2. Press **Save and Run**.

---

#### Updating Your Arduino Code

Before updating the Arduino code, install two libraries that is necessary for `google_drive.cpp`
...
**In the Arduino code (`.ino` file):** Locate `ssid`, `pass`, `myChannelID`,  `writeApiKey`, and the `webAppUrl` constant values and replace the placeholders with your credentials. Save the file and upload the sketch to ESP32-S3.

<img src = "./images/your_thingspeak_writeApiKey.png">

Events below describe what happen when image capture is triggered:

**1. Trigger Image Capture:** by either pressing the IO0 button on the ESP32-S3 board or by sending a command (e.g., 12 12) through the Serial Monitor. The screenshot below shows the expected output.

<img src = "./images/google_new_image_upload.png">

**2. Image Upload to Google Drive:** ESP32 saves the captured image to the SD card and then uploads it to Google Drive via the Google Apps Script. In your Google Drive, the script automatically organizes the images into daily folders, with filenames corresponding to the date and time of capture.

<img src="./images/google_drive_new_image.png">

**3. Receive Public URL:** The Google Apps Script returns a public URL for the image (e.g., https://drive.google.com/uc?export=view&id=...). This URL is web browser compatible that means you can copy and paste the URL *"as-is"* to a browser to display the image.

**4. URL Encoding:** This raw URL is not fully compatible with ThingSpeak, as it will be truncated at the special character `&`. The `urlEncode()` function in `google_drive.cpp` converts the URL into a compatible format by percent-encoding special characters (e.g., & becomes %26).

**5. ThingSpeak Update:** The encoded URL is sent to your ThingSpeak channel.

**6. Image Rendering:** The MATLAB visualization script in ThingSpeak reads the image URL, sends an HTTP GET request to Google Drive, and receives the image data in response. The image is then displayed in the ThingSpeak visualization panel.

<img src="./images/Image_on_ThingSpeak.jpg">

### Part 3: Integrating Image Upload into the Main Application

Once manual trigger works reliably, the final step is to integrate this functionality into the main smart planter application (based on the code from `7_Run_WaterPump_with_Millis_ESP32`).

In the final version, the program sequence is:

1. Read the moisture sensor value.
2. Capture the image (call the snapshot function).
3. Upload the image to Google Drive and get the formatted URL.
4. Send both the moisture value and the image URL to ThingSpeak in a single update.
4. If soil moisture drops below a certain threshold, irrigate the plant by triggering the water pump.
4. Blink the LED in the background.
...
4. Copy the initialization code for the SD Card and Camera parts from project `10_Images_Upload_Google_Drive_ThingSpeak`.

   <img src = "./images/Project_11_copy_the_camera_init_part.png">

   

5. **Optional:** add support for the camera web server by calling `startCameraServer()` after a successful WiFi connection.

   <img src = "./images/Project_11_add_support_for_camerawebServer.png">

6. Create new functions to take a snapshot and upload the image to Google Drive and ThingSpeak. The functions are simplified version of the one used in `10_Images_Upload_GoogleDrive_ThingSpeak.ino`.

   <img src = "./images/Project_11_SnapShotImageUpload.png">

   

7. Integrate the main program loop from `10_Images_Upload_GoogleDrive_ThingSpeak.ino` 

   <img src = "./images/Project_11_Call_SnapShot_when_Soaking.png">

   

**Testing and Verification:** After integrating the code, thoroughly test the system to ensure that the water pump control and image upload functionalities work as expected. You have now successfully added a powerful feature to your smart planter, allowing you to visually monitor your plant's condition remotely.
